...
SKY.Utils = {
    ...
    has2dTransformationSupport: function () {
        null === this.transformation2dSupport && (this.transformation2dSupport =
            Modernizr.csstransforms);
        return this.transformation2dSupport
    },
    has3dTransformationSupport: function () {
        null === this.transformation3dSupport && (this.transformation3dSupport =
            Modernizr.csstransforms3d);
        return this.transformation3dSupport
    },
    hasTransitionSupport: function () {
        null === this.transitionSupport && (this.transitionSupport =
            Modernizr.csstransitions);
        return this.transitionSupport
    },
    getPrefixedProperty: function (a) {
        void 0 === this.prefixedProps[a] && (this.prefixedProps[a] =
            Modernizr.prefixed(a));
    ...
    setCursor: function (a) {
        switch (a) {
            ...
            case "closedhand":
                $("body").css("cursor",
                    "url(images/sc-graphics/closedhand.cur), auto");
                break;
            default:
                $("body").css("cursor", a)
        }
...
SKY.ImageLoader.prototype = {
    ...
    unsubscribe: function (a) {
        for (var b = 0; b < this.subscribers.length; b++) this.subscribers[
            b] === a && delete this.subscribers[b]
    },
    publish: function () {
        if (!this.fired) {
            this.fired = !0;
            for (var a = 0; a < this.subscribers.length; a++)
                if ("function" === typeof this.subscribers[a]) this.subscribers[
                    a]()
        }
    },
    load: function () {
        ...
        this.img.addEventListener ? this.img.addEventListener("load",
            function (b) {
                a.onLoad(b)
            }, !1) : this.img.attachEvent && this.img.attachEvent(
            "onload", function (b) {
                a.onLoad(b)
            });
        if (this.img.complete || void 0 === this.img.complete ||
            "loading" === this.img.readyState) {
            var b = this.img.src;
            this.img.src =
...
SKY.Timer.prototype = {
    ...
    publish: function (a) {
        for (var b = 0; b < this.subscribers.length; b++)
            if ("function" === typeof this.subscribers[b]) this.subscribers[
                b](a)
    },
    reset: function () {
        this.currentCount = 0
    ...
    stop: function () {
        this.running && (clearInterval(this.intervalID), this.running = !
            1)
    },
    tick: function () {
        ++this.currentCount;
        this.publish("timer");
        this.currentCount == this.repeatCount && (this.reset(), this.stop(),
...
SKY.CarouselItem.prototype = {
    init: function () {
        if (!1 == $.support.leadingWhitespace) {
            var a = 2 * parseInt(this.imageElement.css("padding-left")),
                b = 2 * parseInt(this.imageElement.css(
                    "border-left-width")),
    ...
    unsubscribe: function (a, b) {
        for (var c = 0; c < this.subscribers[a].length; c++) this.subscribers[
            a][c] === b && delete this.subscribers[a][c]
    },
    publish: function (a, b) {
        for (var c = 0; c < this.subscribers[a].length; c++)
            if ("function" === typeof this.subscribers[a][c]) this.subscribers[
                a][c](b)
    },
    index: function () {
    ...
    onImageLoaded: function () {
        ...
        this.carousel.settings.reflectionVisible && (this.reflection =
            SKY.CarouselItem.createReflection(this.imageElement, a.reflectionSize,
                a.reflectionAlpha), this.reflection.css({
                    "float": "left",
                    clear: "both",
    ...
    setXY: function (a, b) {
        a != this.x && b != this.y && (this.scaledX += a - this.x, this
            .scaledY += b - this.y, this.x = a, this.y = b, this.update()
        )
    },
    setScale: function (a) {
    ...
    setZIndex: function (a) {
        a != this.zIndex && (this.zIndex = a, this.element.css(
            "z-index", a))
    },
    ...
    updateBaseOffset: function () {
        var a = this.carousel.settings;
        this.baseOffset = this.index() * (a.itemWidth * a.unselectedItemZoomFactor +
            a.distance)
    },
    update: function () {
        var a = this.carousel.settings;
        if (SKY.Utils.has2dTransformationSupport()) a = "translate(" +
            this.x + "px, " + this.y + "px) scale(" + this.scale + ")",
            SKY.Utils.has3dTransformationSupport() && (a +=
                " translateZ(0)"), this.element.css(SKY.Utils.getPrefixedProperty(
                "transform"), a), this.element.css("opacity", this.alpha);
        else {
            ...
            a.reflectionVisible && !SKY.Utils.hasCanvasSupport() ? (c.opacity =
                this.alpha, this.reflection && this.reflection.css({
                    width: b.width,
                    height: b.height,
                    filter: SKY.CarouselItem.getAlphaFilterStatement(
                        a.reflectionAlpha, a.reflectionSize,
                        a.itemHeight)
...
SKY.CarouselItem.createReflection = function (a, b, c) {
    var d = a.width(),
        g = a.height(),
        e = null;
    SKY.Utils.hasCanvasSupport() ? (e = $("<canvas>"), ctx = e.get(0).getContext(
            "2d"), e.attr({
                width: d,
                height: b
            }), e.addClass("reflection"), ctx.save(), ctx.translate(0, g),
        ctx.scale(1, -1), ctx.drawImage(a.get(0), 0, 0, d, g), ctx.restore(),
        ctx.globalCompositeOperation = "destination-out", a = ctx.createLinearGradient(
            0, 0, 0, b), a.addColorStop(0, "rgba(0, 0, 0, " + (1 - c) +
            ")"), a.addColorStop(1, "rgba(0, 0, 0, 1.0)"), ctx.fillStyle =
        a, ctx.fillRect(0, 0, d, b)) : (e = $("<img>"), e.attr({
            src: a.get(0).src
        }), e.css("filter", SKY.CarouselItem.getAlphaFilterStatement(c,
        b, g)));
    return e
};
SKY.CarouselItem.getAlphaFilterStatement = function (a, b, c) {
    return "flipv progid:DXImageTransform.Microsoft.Alpha(opacity=" + 100 *
        a +
...
SKY.Container.prototype = {
    ...
    setTopMargin: function (a) {
        var b = this.carousel.settings,
            c = b.itemHeight;
        "auto" == a && (a = (this.carousel.dom.carousel.height() - c *
            b.selectedItemZoomFactor) / 2);
        a = -c * (1 - b.selectedItemZoomFactor) / 2 + a;
        this.element.css("margin-top", a + "px")
    },
    update: function (a) {
        var b = this;
        a ? (this.carousel.onSelectionAnimationStart(), this.element.on(
            ...
            }), SKY.Utils.hasTransitionSupport() ? (this.element
            .css(SKY.Utils.getPrefixedProperty("transition"),
                "left " + a + "s ease-out"), this.element.css(
                "left", this.x)) : this.element.stop().animate({
                    left: this.x
                }, 1E3 * a, function () {
                    b.carousel.onSelectionAnimationEnd()
                })) : (SKY.Utils.hasTransitionSupport() && this.element.css(
                SKY.Utils.getPrefixedProperty("transition"), ""),
...
SKY.Carousel = function (a, b) {
    this.settings = {
        ...
        gradientStartPoint: 0.10,
        gradientEndPoint: 0.5,
    ...
    this.extraDistanceUnit = this.scaleUnit = this.alphaUnit = this.centerY =
        this.centerX = this.timer = this.preloader = this.contentContainer =
        this.container = this.closestItem = this.selectedItem = null;
    this.carouselItems = [];
    this.dom = {
        carousel: a
    };
...
SKY.Carousel.prototype = {
    ...
    initDOM: function () {
        ...
        this.dom.wrapper = this.dom.carousel.children(
            ".sky-carousel-wrapper");
        this.dom.container = this.dom.wrapper.children(
            ".sky-carousel-container");
        ...
        a.preload && !1 != $.support.leadingWhitespace && (this.dom.wrapper
            .css({
                visibility: "hidden",
                opacity: 0
            }), a.showPreloader && (this.preloader = $(
                    '<div class="sc-preloader"></div>'), this.dom.carousel
    ...
    initConfigParams: function () {
        ...
        SKY.Utils.has2dTransformationSupport() && this.dom.items.css(
            SKY.Utils.getPrefixedProperty("transformOrigin"),
            "center " + Math.round(a.itemHeight / 2) + "px")
    },
    initEvents: function () {
        SKY.Utils.hasTouchSupport() ? (this.events.startEvent =
            "touchstart.sc", this.events.moveEvent = "touchmove.sc",
            this.events.endEvent = "touchend.sc", this.events.touchCancel =
            "touchcancel.sc") : (this.events.startEvent =
            "mousedown.sc", this.events.moveEvent = "mousemove.sc",
    ...
    initContainer: function () {
        ...
        SKY.Utils.hasTouchSupport() || this.dom.carousel.hover(function (
            b) {
            a.mouseOver = !0;
            a.updateCursor()
        }, function (b) {
    ...
    initGradientOverlays: function () {
        ...
        if (a.gradientOverlayVisible) {
            var b = this.createGradientOverlay("left", a.gradientStartPoint,
                    a.gradientEndPoint, a.gradientOverlayColor, a.gradientOverlaySize
                ),
                a = this.createGradientOverlay("right", a.gradientStartPoint,
                    a.gradientEndPoint, a.gradientOverlayColor, a.gradientOverlaySize
                );
    ...
    initContentWrapper: function () {
        ...
        this.contentContainer = $(
            '<div class="sc-content-container"></div>');
        this.contentContainer.append(
            '<div class="sc-content"><section></section></div>');
        a.append(this.contentContainer);
        !1 != $.support.leadingWhitespace && this.settings.preload &&
    ...
    initKeyboardNavigation: function () {
        ...
        s.enableKeyboard && this.dom.document.keydown(function (b) {
            if ("textarea" != b.target.type && "text" != b.target
                .type) switch (b.keyCode) {
                    case 37:
        ...
        if (b.enableMouseWheel) {
            var d = function (c) {
                ...
                c.preventDefault ? c.preventDefault() : (c.returnValue = !
                    1, c.cancelBubble = !0);
                c.wheelDelta ? d = c.wheelDelta / 120 : c.detail &&
                    (d = -c.detail / 3);
                b.reverseMouseWheel && (d *= -1);
                0 < d ? a.selectPrevious(b.slideSpeed) : 0 > d && a
                    .selectNext(b.slideSpeed)
            };
            c.addEventListener ? (c.addEventListener("mousewheel", d, !
                    1), c.addEventListener("DOMMouseScroll", d, !1)) :
                c.attachEvent && c.attachEvent("onmousewheel", d)
        }
    },
    ...
    startAutoSlideshow: function () {
        var a = this,
            b = this.settings;
        this.timer || (this.timer = new SKY.Timer(1E3 * b.autoSlideshowDelay),
            this.timer.subscribe(function (c) {
                a.selectedItem.index() < a.carouselItems.length -
                    1 ? a.selectNext(b.slideSpeed) : b.loop &&
                    a.select(a.carouselItems[0], b.slideSpeed)
            }));
        this.timer.start()
    },
    ...
    setCurrentContent: function (a) {
        0 < a.content.length ? (this.contentContainer.find("section").html(a
                .content.children("section").html()), this.contentContainer
            .find("div").html(a.content.children("div").html())) : (
            this.contentContainer.find("section").empty(), this.contentContainer
            .find("div").empty())
    },
    getStartItem: function () {
        var a = this.settings.startIndex;
        "auto" === a && (a = Math.round(this.carouselItems.length / 2) -
            1);
    ...
    select: function (a, b) {
        ...
        this.selectedItem && this.selectedItem.removeClass(
            "sc-selected");
    ...
    calculateUnits: function () {
        ...
        this.scaleUnit = (a.selectedItemZoomFactor - a.unselectedItemZoomFactor) /
    ...
    update: function () {
        for (var a = this, b = this.settings, c = this.container,
            ...
            var f = this.carouselItems[e],
                ...
                k = Math.abs(h);
            if (k <= b.motionStartDistance) var p = this.calculateAlpha(
                    k),
                l = this.calculateScale(k),
                n = this.calculateExtraDistance(k);
            else p = b.unselectedItemAlpha, l = b.unselectedItemZoomFactor,
                n = 0;
            k <= this.centerX && (f.setDistance(k), g.push(f));
            f.setAlpha(p);
            f.setScale(l);
            0 < h ? f.setX(f.getBaseOffset() + 2 * b.selectedItemDistance -
                n) : f.setX(f.getBaseOffset() + n);
            h = k = p = l = n = null
        }
        this.zSort(g);
    ...
    updateLayout: function () {
        ...
        this.container.setX(this.centerX - this.selectedItem.x - a.itemWidth /
            2)
    },
    updateCursor: function () {
        this.dragging ? SKY.Utils.setCursor("closedhand") : this.mouseOver ?
            SKY.Utils.setCursor("openhand") : SKY.Utils.setCursor(
                "auto")
    },
    calculateAlpha: function (a) {
        return (this.settings.motionStartDistance - a) * this.alphaUnit +
            this.settings.unselectedItemAlpha
    },
    calculateScale: function (a) {
        return (this.settings.motionStartDistance - a) * this.scaleUnit +
            this.settings.unselectedItemZoomFactor
    },
    calculateExtraDistance: function (a) {
        return Math.ceil((this.settings.motionStartDistance - a) * this
            .extraDistanceUnit)
    },
    onStart: function (a) {
        function b(a) {
            var b = a.originalEvent,
                c = SKY.Utils.hasTouchSupport() ? b.touches[0].clientX :
                a.clientX,
                g = SKY.Utils.hasTouchSupport() ? b.touches[0].clientY :
                a.clientY,
                r = (c - startX) / 2 + n;
            m = f - c;
            f = c;
            if (SKY.Utils.hasTouchSupport()) {
                ...
                t || (t = !0, Math.abs(g - h) > Math.abs(c - startX) +
                    5 ? d.isScrolling = !0 : d.isScrolling = !1
                );
                if (d.isScrolling) {
                    q = !0;
                    return
        ...
        function c(a) {
            var f = a.originalEvent,
                e = SKY.Utils.hasTouchSupport() ? f.changedTouches[
                    0].clientX : a.clientX;
            a = SKY.Utils.hasTouchSupport() ? f.changedTouches[0].clientY :
                a.clientY;
            ...
            d.events.touchCancel && d.dom.document.off(d.events.touchCancel,
                c);
            0 == Math.abs(startX - e) ? (e = $(document.elementFromPoint(
                    e, a)), e.hasClass("sc-image") ? (e.parent()
                    .is("a") && (e = e.parent()), e = d.carouselItems[
                        e.parent().index()], g.selectByClick &&
                    e !== d.selectedItem ? d.select(e, g.slideSpeed) :
                    g.selectByClick && e === d.selectedItem &&
                    d.selectNext(g.slideSpeed)) : g.selectByClick &&
                d.selectNext(g.slideSpeed)) : (0 == f && (0 <
                    Math.abs(m) && d.closestItem.index() == d.selectedItem
                    .index()) && (f = 0 < m ? 1 : 0 > m ? -1 :
                    0), h += f, h = 0 > h ? 0 : h > k - 1 ? k -
                1 : h, !u && !q && d.select(h, g.slideSpeed));
            d.dragging = !1;
            d.updateCursor()
        }
        if ("A" != a.target.nodeName) {
            var d = this,
                g = this.settings,
                e = a.originalEvent,
                f = startX = SKY.Utils.hasTouchSupport() ? e.targetTouches[
                    0].clientX : a.clientX,
                h = SKY.Utils.hasTouchSupport() ? e.targetTouches[0].clientY :
                a.clientY,
                k = this.carouselItems.length,
                p = d.centerX - (g.selectedItemDistance + g.itemWidth /
                    2),
                l = p - (g.itemWidth * g.unselectedItemZoomFactor + g.distance) *
                (k - 1),
            ...
            if (this.events.touchCancel) this.dom.document.on(this.events
                .touchCancel, c);
            SKY.Utils.hasTouchSupport() ? 1 < e.touches.length && (q = !
                0) : a.preventDefault()
        }
    },
    onAllLoaded: function () {
        ...
        if (!1 != $.support.leadingWhitespace) {
            ...
            b.preload && (b.showPreloader ? this.preloader.fadeOut(700,
                c) : c())
        }
    },
    on: function (a, b) {
    ...
    createGradientOverlay: function (a, b, c, d, g) {
        if (SKY.Utils.hasCanvasSupport()) {
            var e = $('<canvas class="sc-overlay" width="' + g +
                    '" height="1"></canvas'),
            ...
            "left" == a ? h = f.createLinearGradient(0, 0, g, 0) :
                "right" == a && (h = f.createLinearGradient(g, 0, 0, 0));
            h.addColorStop(b, "rgba(" + d.r + ", " + d.g + ", " + d.b +
                ", 1.0)");
            h.addColorStop(c, "rgba(" + d.r + ", " + d.g + ", " + d.b +
                ", 0)");
            f.fillStyle = h;
            f.fillRect(0, 0, g, 1);
            return e
...
(function (a) {
    a.fn.carousel = function (b) {
        ...
        this.each(function () {
            var d = a(this);
            d.data("sky-carousel") || d.data("sky-carousel",
                new SKY.Carousel(d, b));
...
window.Modernizr = function (a, b, c) {
    ...
    var f = {},
        ...
        u, w = function (a, c, d, e) {
            ...
            if (parseInt(d, 10))
                for (; d--;) k = b.createElement("div"), k.id = e ? e[d] :
                    "modernizr" + (d + 1), l.appendChild(k);
            return f = ['&#173;<style id="smodernizr">', a, "</style>"].join(
                    ""), l.id = "modernizr", (n ? l : m).innerHTML += f, m.appendChild(
                    l), n || (m.style.background = "", m.style.overflow =
                    "hidden", p = h.style.overflow, h.style.overflow =
                    "hidden", h.appendChild(m)), g = c(l, a), n ? l.parentNode
                .removeChild(l) : (m.parentNode.removeChild(m), h.style.overflow =
                    p), !!g
        },
        x = {}.hasOwnProperty,
        v;
    ...
    k.canvastext = function () {
        return !!f.canvas && !!d(b.createElement("canvas").getContext(
    ...
    k.csstransforms3d = function () {
        var a = !!e("perspective");
        return a && "webkitPerspective" in h.style && w(
    ...
    for (var y in k) v(k, y) && (u = y.toLowerCase(), f[u] = k[y](), t.push(
        (f[u] ? "" : "no-") + u));
    ...
    p.cssText = "";
    return k = null, f._version = "2.6.2", f._prefixes = l, f._domPrefixes =
        m, f._cssomPrefixes = n, f.testProp = function (a) {
            return g([a])
        }, f.testAllProps = e, f.testStyles = w, f.prefixed = function (a, b,
            c) {
            return b ? e(a, b, c) : e(a, "pfx")
        }, h.className = h.className.replace(/(^|\s)no-js(\s|$)/, "$1$2") +
        (" js " + t.join(" ")), f
}(this, this.document);